export const convertTodaysData = (data) => {
  ...
  if (data.length > 0) {
    ...
    result = data.map((ele) => {
      ...
      const firstShiftComments = firstShiftOTG?.comments?.concat(
        firstShiftDrivers?.comments
      );
      const secondShiftComments = secondShiftOTG?.comments?.concat(
        secondShiftDrivers?.comments
      );
      const breakdownComments = breakdownCar?.comments?.concat(
        breakdownVan?.comments
      );

      const setupTime =
        (setupVan?.starthour % 12) +
        ":" +
        setupVan?.startminute.toString().padStart(2, "0") +
        " - " +
        (setupVan?.endhour % 12) +
        ":" +
        setupVan?.endminute.toString().padStart(2, "0");
      const firstShiftTime =
        (firstShiftOTG?.starthour % 12) +
        ":" +
        firstShiftOTG?.startminute.toString().padStart(2, "0") +
        " - " +
        (firstShiftOTG?.endhour % 12) +
        ":" +
        firstShiftOTG?.endminute.toString().padStart(2, "0");
      const secondShiftTime =
        (secondShiftOTG?.starthour % 12) +
        ":" +
        secondShiftOTG?.startminute.toString().padStart(2, "0") +
        " - " +
        (secondShiftOTG?.endhour % 12) +
        ":" +
        secondShiftOTG?.endminute.toString().padStart(2, "0");
      const breakdownTime =
        (breakdownVan?.starthour % 12) +
        ":" +
        breakdownVan?.startminute.toString().padStart(2, "0") +
        " - " +
        (breakdownVan?.endhour % 12) +
        ":" +
        breakdownVan?.endminute.toString().padStart(2, "0");
      ...
      container.setupComments =
        setupComments?.length > 0 ? setupComments : null;
      ...
      container.firstShiftOTGCount = firstShiftOTGCount
        ? firstShiftOTGCount
        : null;
      container.firstShiftComments =
        firstShiftComments?.length > 0 ? firstShiftComments : null;
      ...
      container.secondShiftOTGCount = secondShiftOTGCount
        ? secondShiftOTGCount
        : null;
      container.secondShiftComments =
        secondShiftComments?.length > 0 ? secondShiftComments : null;
      ...
      container.breakdownVanCount = breakdownVanCount
        ? breakdownVanCount
        : null;
      container.breakdownComments =
        breakdownComments?.length > 0 ? breakdownComments : null;
