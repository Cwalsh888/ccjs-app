export const convertData = (data) => {
  ...
  if (data.length > 0) {
    result = data.map(ele => {
      ...
      let halfDay = (legacyFirstShiftDriverCount + legacyFirstShiftOTGCount >= 2 || firstShiftDriverCount + firstShiftOTGCount >= 2) ||
                    (legacySecondShiftDriverCount + legacySecondShiftOTGCount >= 2 || secondShiftDriverCount + secondShiftOTGCount >= 2);
      let fullDay = (legacyFirstShiftDriverCount + legacyFirstShiftOTGCount >= 2 || firstShiftDriverCount + firstShiftOTGCount >= 2) &&
                    (legacySecondShiftDriverCount + legacySecondShiftOTGCount >= 2 || secondShiftDriverCount + secondShiftOTGCount >= 2); 
