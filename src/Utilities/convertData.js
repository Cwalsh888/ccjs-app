export const convertData = (data) => {
    ...
    if (data.length > 0) {
      result = data.map(ele => {
        ...
        let setupComments = ele.jobs.find(ele => ele.name === 'set up van driver')?.comments?.concat(ele.jobs.find(ele => ele.name === 'set up car (daily supply needs pickup)')?.comments);
